...
/** @brief uint8 CC, uint8 CmdCnt, uint8 CmdErrCnt, uint16 SubCnt,
 * uint16 NetCnt
 */
#define SBN_HK_LEN ((CFE_SB_TLM_HDR_SIZE) + (sizeof(uint8) * 3) + (sizeof(uint16) * 2))
...
/** @brief uint8 CC, uint8 NetIdx, uint8 PeerIdx, uint16 SubCnt,
 * CFE_SB_MsgId_t Subs[SBN_MAX_SUBS_PER_PEER]
 */
#define SBN_HKPEERSUBS_LEN (CFE_SB_TLM_HDR_SIZE + sizeof(uint8) * 3 + sizeof(uint16) * 2 + SBN_MAX_SUBS_PER_PEER * sizeof(CFE_SB_MsgId_t))

/** @brief uint8 CC, uint8 QoS, uint8 SubCnt, uint8 SBN_MAX_NET_NAME_LEN,
 ...
 * uint8 ProtocolID, uint8 PeerCnt
 */
#define SBN_HKNET_LEN (CFE_SB_TLM_HDR_SIZE + sizeof(uint8) * 2 + sizeof(char) * SBN_MAX_NET_NAME_LEN + sizeof(uint8) * 2)
