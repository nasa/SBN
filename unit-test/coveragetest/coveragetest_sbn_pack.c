...
void Test_Pack(void)
{
    ...
    uint8 u8; UtAssert_True(Unpack_UInt8(&Pack, &u8), "unpack uint8"); // 1 byte
    UtAssert_UINT32_EQ(u8, 1);

    UtAssert_True(Unpack_UInt8(&Pack, &u8), "unpack uint8"); // 2 bytes
    UtAssert_UINT32_EQ(u8, 255);

    uint16 u16; UtAssert_True(Unpack_UInt16(&Pack, &u16), "unpack uint16"); // 4 bytes
    UtAssert_UINT32_EQ(u16, 2);

    int16 i16; UtAssert_True(Unpack_Int16(&Pack, &i16), "unpack int16"); // 6 bytes
    UtAssert_INT32_EQ(i16, -2);

    uint32 u32; UtAssert_True(Unpack_UInt32(&Pack, &u32), "unpack uint32"); // 10 bytes
    UtAssert_UINT32_EQ(u32, 3);

    CFE_SB_MsgId_t MsgID; UtAssert_True(Unpack_MsgID(&Pack, &MsgID), "unpack msgid"); // 12 bytes
